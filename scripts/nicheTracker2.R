...
#run analyses in parallel (~6gb memory per core; swap ok on mbp)
registerDoMC(cores=4) 

t1 <- Sys.time()
#shortniche(levels(gbif$species)[1])
...
hummers <- data.frame(fread("Trochilidae_nicheTracker2.csv",drop=1))
names(hummers) <- c("species","centroid.distance", "I.obs", "I.res.s", "I.res.w", "p.similar","migstatus")
